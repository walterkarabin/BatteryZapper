...
public class LaserGun : MonoBehaviour
{
    ...
    {
        RaycastHit hit;
        if (Physics.Raycast(muzzle.transform.position, muzzle.transform.right, out hit, Mathf.Infinity, layerMask))
        {
            Draw2DRay(muzzle.transform.position, hit.point * 0.99f);
            if (hit.transform.tag == "Battery")
            {
                ChargeBattery(hit.transform.gameObject);
            }
        }
        else
        {
    ...
    private void ChargeBattery(GameObject obj)
    {
        if(Time.time - lastCharge > 0.2f)
        {
            obj.gameObject.GetComponent<Battery>().Charging(chargeSpeed);
